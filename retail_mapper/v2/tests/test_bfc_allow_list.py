...
def test_every_row_in_bfc_allow_list(audit_rows, bfc_allowed_paths):
    ...
    for its BFC. Skips BFCs not in the allow-list (rare/tiny ones)."""
    real = {k: v for k, v in bfc_allowed_paths.items() if not k.startswith("_")}
    bad = []
    n_checked = 0
    n_skipped = 0
    for r in audit_rows:
        ...
        # Allow exact match or any allowed prefix that the path starts with
        allowed = real[bfc]
        if top2 in allowed:
            continue
    ...
    if bad:
        fail_with_samples(
            f"Invariant 25 violated: {len(bad):,} SKUs have canonical_path not in BFC allow-list",
