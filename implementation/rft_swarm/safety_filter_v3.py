"""Safety filter v3 with agent-learned rules."""
import csv, sys
...
def is_rejected(row):
    ...
    for f1, p1, f2, p2, reason in REJECT_PATTERNS:
        ...
        if reason in ('reject_rootbeer_to_beer', 'reject_gingerbeer_to_beer'):
            continue
        # Cheese curls: only reject if product actually contains cheese
        if reason == 'reject_cheese_curls' and 'cheese' in (row.get('product_description') or '').lower():
            return reason
        if reason == 'reject_cheese_curls':
            continue
        # Yogurt to milk: only if product contains yogurt
        if reason == 'reject_yogurt_to_milk' and 'yogurt' in (row.get('product_description') or '').lower():
            return reason
        if reason == 'reject_yogurt_to_milk':
            continue
        # Candy rules: only if category is candy
        if reason.startswith('reject_candy_') and 'candy' in (row.get('branded_food_category') or '').lower():
            return reason
        if reason.startswith('reject_candy_'):
            continue
        # Dairy to plant: only if product has milk but not soy/oat/almond
