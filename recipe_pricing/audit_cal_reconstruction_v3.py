#!/usr/bin/env python3
"""A3 v3 — Calorie reconstruction with hestia_fndds + ingredient_lookup fallback.

v2 used only hestia_fndds (16.4% line-level miss because Hestia leaves the
attribution empty on some lines). v3 falls back to Hestia's name→fndds map
in ingredient_lookup.json when hestia_fndds is missing.

Cal lookup remains OUR fndds_nutrient_lookup.csv (10,778 entries).

Read-only.

Outputs:
  recipe_pricing/audit_cal_reconstruction_v3.csv
"""
...
OUT = ROOT / "recipe_pricing" / "audit_cal_reconstruction_v3.csv"


def main():
    print("loading FNDDS nutrient lookup (energy_kcal/100g)…", file=sys.stderr)
    # Include kcal=0 entries (salt, spices, baking soda) so they count as
    # "have data" rather than missing.  They contribute 0 cal anyway.
    ...
    n_zero = sum(1 for v in fndds_kcal.values() if v == 0)
    print(f"  {len(fndds_kcal):,} FNDDS codes "
          f"({len(fndds_kcal)-n_zero:,} >0 kcal, {n_zero:,} zero-cal foods)",
          file=sys.stderr)

    print("loading ingredient_lookup (name→fndds fallback)…", file=sys.stderr)
    ...
    print(f"  {len(name_to_fndds):,} name→fndds entries", file=sys.stderr)
    ...
    per_recipe: dict[str, dict] = defaultdict(
        lambda: {"our_cal": 0.0, "missing": 0, "n_lines": 0,
                  "name": "", "top_contrib": []})
    n_via_hes = 0; n_via_lookup = 0; n_no_fndds = 0; n_no_kcal = 0
    ...
    print(f"  via hestia_fndds:          {n_via_hes:,}", file=sys.stderr)
    print(f"  via ingredient_lookup:     {n_via_lookup:,}", file=sys.stderr)
    print(f"  no fndds (counted miss):   {n_no_fndds:,}", file=sys.stderr)
    print(f"  no kcal entry:             {n_no_kcal:,}", file=sys.stderr)

    print(f"\nFlag distribution (audit v3):", file=sys.stderr)
    ...
    print(f"\nLine miss rate: {total_missing*100/max(1,total_lines):.1f}% "
          f"({total_missing:,} of {total_lines:,})", file=sys.stderr)
