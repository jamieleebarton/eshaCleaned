...
class ScoringConfig:
    ...
    fresh_herb_micro_use_max_g: float = 4.0  # Tiny fresh-herb asks are treated as seasoning portions
    fresh_herb_micro_package_g: float = 4.0  # Modeled package for <=4g garnish/seasoning herb use
    fresh_herb_micro_price_per_g: float = 0.10  # Approx dried-herb/spice price for tiny herb asks
    fresh_herb_micro_shelf_days: int = 365  # Micro herb portions behave like dried/pantry seasoning
    ...
    enable_at_risk_pantry_rescue: bool = False  # Experiment: extra credit for burning near-expiry pantry
    at_risk_rescue_ttl_days: int = 3          # Only inventory expiring within this many days gets rescue credit
    at_risk_rescue_fresh_value_per_g: float = 0.04  # $/gram credit for fresh at-risk pantry overlap
    at_risk_rescue_frozen_value_per_g: float = 0.015  # $/gram credit for frozen quality-window overlap
    at_risk_rescue_max_bonus_per_recipe: float = 8.0  # Cap so rescue cannot dominate nutrition/cost
    at_risk_rescue_new_perishable_penalty_per_g: float = 0.015  # Penalize new perishable package remainders
    at_risk_rescue_new_perishable_ttl_days: int = 14  # Remainders with TTL <= this count as new risk
    ...
    excess_serving_penalty: float = 1.50   # $ penalty multiplier per excess serving

    # === BONUS PARAMETERS ===
    template_match_bonus: float = 3.0      # $/match for side leftovers matching main template
    frozen_pressure_base: float = 12.0     # Base penalty when frozen items accumulate
    ...
    leftover_excess_threshold_mult: float = 4.0  # Penalty triggers at slot_servings * this
    leftover_waste_penalty: float = 5.0      # $ penalty per leftover serving that expires
    pantry_spoilage_kg_penalty: float = 12.0  # $ penalty per kg pantry spoilage
    freezer_over_capacity_kg_penalty: float = 35.0  # $ penalty per kg above freezer capacity
    ...
    urgent_leftover_daily_ceiling_pct: float = 1.10  # Urgent leftovers may bypass pacing, not daily overfeed cap
    leftover_slot_ceiling_pct: float = 1.10  # Normal leftover portions cannot overfeed the current meal slot
    urgent_leftover_slot_ceiling_pct: float = 1.10  # Urgent leftovers get a little slot slack, not unlimited meals
    ...
    def balanced(cls, protein_diversity: bool = False) -> 'ScoringConfig':
        ...
        config = cls(
            ...
            # Small fruit snacks can top up baseline 15% protein plans that
            # survive beam search just below the weekly calorie floor.
            enable_fruit_snacks=True,
            snack_inject_floor=0.85,
            snack_inject_ceiling=0.95,
        )
        return config
    ...
    # =========================================================================
    # These presets implement USDA-style food plan tiers using the in-loop
    # protein-source quota. Tier = protein-source mix, not cosmetic swaps after
    # accounting has already been computed.
    #
    ...
    def thrifty(cls, protein_pct: float = 15.0) -> 'ScoringConfig':
        """Thrifty tier: baseline cost optimizer.

        Leaves protein-source quota off so the cheapest feasible plan wins.
    ...
    def low_cost(cls, protein_pct: float = 15.0) -> 'ScoringConfig':
        ...
        Uses quota to steer toward chicken and pork while keeping eggs/legumes
        available as cheap anchors.
        ...
        config.protein_pct_target = protein_pct
        config.tier_config = None
        config.protein_target_distribution = [0.03, 0.18, 0.30, 0.04, 0.35, 0.10]
        config.protein_quota_strictness = 7.0
        config.enable_fruit_snacks = True
        config.snack_inject_floor = 0.85
        config.snack_inject_ceiling = 0.95
        config.enable_produce_bonus = True
        config.produce_value_breakfast = 0.000
        config.produce_value_lunch = 0.002
        config.produce_value_dinner = 0.005
    ...
    def moderate(cls, protein_pct: float = 15.0) -> 'ScoringConfig':
        ...
        Uses quota to steer toward chicken, pork, and some beef.
        ...
        config.protein_pct_target = protein_pct
        config.tier_config = None
        config.protein_target_distribution = [0.18, 0.16, 0.28, 0.06, 0.24, 0.08]
        config.protein_quota_strictness = 8.0
        config.enable_fruit_snacks = True
        config.snack_inject_floor = 0.85
        config.snack_inject_ceiling = 0.95
        config.enable_produce_bonus = True
    ...
    def liberal(cls, protein_pct: float = 15.0) -> 'ScoringConfig':
        ...
        Uses quota to steer toward more beef and fish while preserving enough
        cheaper proteins to keep the plan feasible.
        ...
        config.protein_pct_target = protein_pct
        config.tier_config = None
        config.protein_target_distribution = [0.22, 0.12, 0.18, 0.22, 0.18, 0.08]
        config.protein_quota_strictness = 9.0
        config.enable_fruit_snacks = True
        config.snack_inject_floor = 0.85
        config.snack_inject_ceiling = 0.95
        config.enable_produce_bonus = True
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'fresh_herb_micro_use_max_g': self.fresh_herb_micro_use_max_g,
            'fresh_herb_micro_package_g': self.fresh_herb_micro_package_g,
            'fresh_herb_micro_price_per_g': self.fresh_herb_micro_price_per_g,
            'fresh_herb_micro_shelf_days': self.fresh_herb_micro_shelf_days,
            ...
            'enable_at_risk_pantry_rescue': self.enable_at_risk_pantry_rescue,
            'at_risk_rescue_ttl_days': self.at_risk_rescue_ttl_days,
            'at_risk_rescue_fresh_value_per_g': self.at_risk_rescue_fresh_value_per_g,
            'at_risk_rescue_frozen_value_per_g': self.at_risk_rescue_frozen_value_per_g,
            'at_risk_rescue_max_bonus_per_recipe': self.at_risk_rescue_max_bonus_per_recipe,
            'at_risk_rescue_new_perishable_penalty_per_g': self.at_risk_rescue_new_perishable_penalty_per_g,
            'at_risk_rescue_new_perishable_ttl_days': self.at_risk_rescue_new_perishable_ttl_days,
            ...
            'leftover_waste_penalty': self.leftover_waste_penalty,
            'pantry_spoilage_kg_penalty': self.pantry_spoilage_kg_penalty,
            'freezer_over_capacity_kg_penalty': self.freezer_over_capacity_kg_penalty,
