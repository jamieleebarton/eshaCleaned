...
FROZEN_SHELF_DAYS = 120  # Conservative freezer quality window, not safety spoilage

FRESH_HERB_PREFIXES = (
    "75109400",  # basil
    "751095",    # cilantro, chives, coriander leaves
    "75119000",  # parsley
)
FRESH_HERB_EXACT_CODES = {
    "94302045",  # fresh dill
    "94302049",  # fresh thyme
    "94302065",  # fresh mint
}


def is_fresh_herb_fpid(code: str) -> bool:
    """Return True for FNDDS codes modeled as fresh herbs."""
    code = str(code or "")
    return code in FRESH_HERB_EXACT_CODES or any(
        code.startswith(prefix) for prefix in FRESH_HERB_PREFIXES
    )
...
class PerishabilityIndex:
    ...
    def __init__(self, perishability_map_path: str = None, device: torch.device = None):
        ...
        self.frozen_shelf_days = self.map.get("frozen_shelf_days", FROZEN_SHELF_DAYS)
...
class PackageIndex:
    ...
    FROZEN_RETAIL_PACKAGE_SHELF_DAYS = 365.0
    SHELF_STABLE_CONDIMENT_PACKAGE_SHELF_DAYS = 365.0
    REFRIGERATED_CONDIMENT_PACKAGE_SHELF_DAYS = 60.0
    ...
    def __init__(self, packages_csv: str = None, packages_db: str = None):
        ...
        self._package_storage_by_key: Dict[Tuple[str, float, float, str], Tuple[float, bool]] = {}
        ...
        if db_path.exists():
            ...
            for row in conn.execute(
                ...
                SELECT fndds_code, food_description, package_weight_grams,
                       kroger_price_cents, walmart_price_cents,
                       package_size_display, product_meta, source
                FROM packages
                """
            ).fetchall():
                code, food_description, grams, kr, wm, size_display, product_meta_raw, source = row
                price_cents = kr or wm or 0
                if price_cents <= 0 or grams <= 0:
                    continue
                try:
                    product_meta = json.loads(product_meta_raw) if product_meta_raw else {}
                except Exception:
                    product_meta = {}
                if not self._package_matches_fndds(code, food_description or "", product_meta):
                    continue
                price = price_cents / 100.0
                price, grams, size_display = self._normalize_purchase_unit(
                    price,
                    grams,
                    size_display or "",
                    source or "",
                ...
                storage = self._infer_package_storage(
                    size_display or "",
                    source or "",
                    product_meta,
                )
                ...
                self._package_storage_by_key[
                    self._package_key(code, price, grams, size_display or "")
                ] = storage
            conn.close()
        ...
        self._gpu_package_shelf_life_days: Optional[torch.Tensor] = None  # [num_ingredients], -1=no override
        self._gpu_package_starts_frozen: Optional[torch.Tensor] = None     # [num_ingredients]
        self._gpu_option_shelf_life_days: Optional[torch.Tensor] = None  # [num_ingredients, MAX_PACKAGE_OPTIONS]
        self._gpu_option_starts_frozen: Optional[torch.Tensor] = None     # [num_ingredients, MAX_PACKAGE_OPTIONS]

    @staticmethod
    def _package_key(code: str, price: float, grams: float, display: str) -> Tuple[str, float, float, str]:
        return (str(code), round(float(price), 6), round(float(grams), 3), str(display or ""))

    @staticmethod
    def _package_text(product_meta: Dict) -> Tuple[str, str, str]:
        name = " ".join(
            str(product_meta.get(k) or "")
            for k in (
                "name",
                "walmart_name",
                "brand",
        ...
        temperature = str(product_meta.get("temperature") or "").lower()
        categories_raw = product_meta.get("categories") or product_meta.get("category_path") or []
        if isinstance(categories_raw, str):
            categories = categories_raw.lower()
        else:
            categories = " ".join(str(c) for c in categories_raw).lower()
        return name, temperature, categories

    @classmethod
    def _infer_package_storage(
        cls,
        size_display: str,
        source: str,
        product_meta: Dict,
    ) -> Tuple[float, bool]:
        """Infer storage behavior from the selected retail package.

        Perishability by FNDDS code is still the default. This only overrides
        obvious package forms where the retail item is frozen or long-life after
        opening, so frozen spinach does not get thrown out as fresh spinach.
        """
        name, temperature, categories = cls._package_text(product_meta)
        text = " ".join([name, temperature, categories, str(size_display or "").lower(), str(source or "").lower()])

        if "frozen" in temperature or "frozen" in categories or " frozen " in f" {text} ":
            return cls.FROZEN_RETAIL_PACKAGE_SHELF_DAYS, True

        shelf_stable_condiment_markers = (
            "pickled",
        ...
        if any(marker in text for marker in shelf_stable_condiment_markers):
            return cls.SHELF_STABLE_CONDIMENT_PACKAGE_SHELF_DAYS, False

        refrigerated_condiment_markers = (
        ...
        if any(marker in text for marker in refrigerated_condiment_markers):
            return cls.REFRIGERATED_CONDIMENT_PACKAGE_SHELF_DAYS, False

        return -1.0, False

    @classmethod
    def _package_choice_key(
        cls,
        fndds: str,
        package: Tuple[float, float, str],
    ) -> Tuple[float, float, float, float]:
        price, grams, _display = package
        ref = cls.REFERENCE_GRAMS_BY_PREFIX.get(str(fndds)[:2], 500)
        package_count = math.ceil(ref / grams)
        purchase_cost = package_count * price
        excess = max(package_count * grams - ref, 0.0)
    ...
    @staticmethod
    def _package_matches_fndds(code: str, food_description: str, product_meta: Dict) -> bool:
        """Reject obvious fresh/dried package mismatches before planner pricing.

        The package DB can contain broad retail matches like cilantro-lime rice
        for fresh cilantro or fresh parsley for dried parsley. Those rows are
        valid retail products, but not valid substitutes for the FNDDS ingredient
        the planner is buying.
        """
        code = str(code or "")
        desc = str(food_description or "").lower()
        name = " ".join(
            str(product_meta.get(k) or "")
            for k in ("name", "walmart_name", "brand", "walmart_brand", "ingredient_statement")
        ).lower()
        temperature = str(product_meta.get("temperature") or "").lower()
        categories_raw = product_meta.get("categories") or product_meta.get("category_path") or []
        if isinstance(categories_raw, str):
            ...
            categories = " ".join(str(c) for c in categories_raw).lower()
        product_text = " ".join([name, temperature, categories])
        text = " ".join([desc, product_text])

        is_fresh_herb = is_fresh_herb_fpid(code)
        is_dried_herb_or_spice = (
            not is_fresh_herb
            and (
                "dried" in desc
                or code.startswith("94302")
                or code.startswith("94303")
            )
        )

        dried_markers = (
            "dried", "dry ", "flakes", "shaker", "ground", "powder",
            "seasoning", "spice", "spices", "leaves shaker",
        )
        fresh_markers = (
            "fresh", "refrigerated", "produce", "bunch", "clamshell",
            "stir-in paste", "herb paste",
        )
        non_herb_product_markers = (
            " rice", "pasta", "grain", "dressing", "sauce", "soup",
            "canned", "packaged",
        )

        if is_dried_herb_or_spice:
            looks_fresh = any(marker in product_text for marker in fresh_markers)
            looks_dried = any(marker in product_text for marker in dried_markers)
            if looks_fresh and not looks_dried:
                return False

        if is_fresh_herb:
            looks_fresh = any(marker in product_text for marker in fresh_markers)
            looks_dried = any(marker in product_text for marker in dried_markers)
            looks_non_herb = any(marker in product_text for marker in non_herb_product_markers)
            if looks_non_herb and not looks_fresh:
                return False
            if looks_dried and not looks_fresh:
                return False
            if not looks_fresh:
                return False

        # Pepper ingredients should not match cheese/dairy products that merely
        # contain peppers as a flavor inclusion.
        if code.startswith("755") and ("dairy" in categories or "cheese" in name):
            return False

        return True

    @staticmethod
    def _normalize_purchase_unit(
        price: float,
        grams: float,
        size_display: str,
        source: str,
        ...
        """Normalize per-pound random-weight rows before package tensors.

        Kroger meat rows often expose a random observed pack weight, but the
        display/price is "1 lb" per pound. Treating those as fixed packs makes
        the planner think a 3-4 lb roast costs the one-pound shelf price.
        """
        display = str(size_display or "").strip().lower()
        if display not in {"1 lb", "1lb"}:
            return price, grams, size_display

        one_lb_g = 453.592
        if 0.90 * one_lb_g <= grams <= 1.10 * one_lb_g:
            return price, grams, size_display

        source_l = str(source or "").lower()
        upc = str(
            product_meta.get("upc")
            or product_meta.get("gtin")
            or product_meta.get("walmart_upc")
            or ""
        )
        categories_raw = product_meta.get("categories") or product_meta.get("category_path") or []
        if isinstance(categories_raw, str):
        ...
        is_kroger_random_weight = source_l.startswith("kroger") and upc.startswith("002")
        is_meat_or_seafood = "meat & seafood" in categories or "meat" in categories
        if is_kroger_random_weight or is_meat_or_seafood:
            return price, one_lb_g, "1 lb"
    ...
    def build_gpu_tensors(
        ...
        package_shelf_life_days = torch.full((num_ing,), -1.0, dtype=torch.float32, device=device)
        package_starts_frozen = torch.zeros((num_ing,), dtype=torch.bool, device=device)
        ...
        option_shelf_life_days = torch.full(
            (num_ing, self.MAX_PACKAGE_OPTIONS),
            -1.0,
            dtype=torch.float32,
            device=device,
        )
        option_starts_frozen = torch.zeros(
            (num_ing, self.MAX_PACKAGE_OPTIONS),
            dtype=torch.bool,
            device=device,
        )
        ...
        for idx in range(num_ing):
            ...
            if fndds in self.packages_by_fndds:
                ...
                pkg = min(
                    pkgs,
                    key=lambda p: self._package_choice_key(
                        fndds,
                        p,
                    ),
                )
                prices[idx] = pkg[0]
                sizes[idx] = pkg[1]
                shelf_days, starts_frozen = self._package_storage_by_key.get(
                    self._package_key(fndds, pkg[0], pkg[1], pkg[2]),
                    (-1.0, False),
                )
                package_shelf_life_days[idx] = float(shelf_days)
                package_starts_frozen[idx] = bool(starts_frozen)
                option_prices[idx, :] = pkg[0]
                option_sizes[idx, :] = pkg[1]
                option_shelf_life_days[idx, :] = float(shelf_days)
                option_starts_frozen[idx, :] = bool(starts_frozen)
                ...
                for opt_idx, (price, grams, _display) in enumerate(deduped):
                    ...
                    opt_shelf_days, opt_starts_frozen = self._package_storage_by_key.get(
                        self._package_key(fndds, price, grams, _display),
                        (-1.0, False),
                    )
                    option_shelf_life_days[idx, opt_idx] = float(opt_shelf_days)
                    option_starts_frozen[idx, opt_idx] = bool(opt_starts_frozen)
                priced += 1
            else:
                defaulted += 1
        ...
        self._gpu_package_shelf_life_days = package_shelf_life_days
        self._gpu_package_starts_frozen = package_starts_frozen
        self._gpu_option_shelf_life_days = option_shelf_life_days
        self._gpu_option_starts_frozen = option_starts_frozen
    ...
    def extend_with_synthetic(
        ...
        new_shelf_days = torch.full((num_new,), -1.0, dtype=torch.float32)
        new_starts_frozen = torch.zeros((num_new,), dtype=torch.bool)
        ...
        new_option_shelf_days = torch.full(
            (num_new, self.MAX_PACKAGE_OPTIONS),
            -1.0,
            dtype=torch.float32,
        )
        new_option_starts_frozen = torch.zeros((num_new, self.MAX_PACKAGE_OPTIONS), dtype=torch.bool)
        ...
        if self._gpu_package_shelf_life_days is not None:
            self._gpu_package_shelf_life_days = torch.cat([
                self._gpu_package_shelf_life_days,
                new_shelf_days.to(device),
            ])
        if self._gpu_package_starts_frozen is not None:
            self._gpu_package_starts_frozen = torch.cat([
                self._gpu_package_starts_frozen,
                new_starts_frozen.to(device),
            ])
        if self._gpu_option_shelf_life_days is not None:
            self._gpu_option_shelf_life_days = torch.cat([
                self._gpu_option_shelf_life_days,
                new_option_shelf_days.to(device),
            ])
        if self._gpu_option_starts_frozen is not None:
            self._gpu_option_starts_frozen = torch.cat([
                self._gpu_option_starts_frozen,
                new_option_starts_frozen.to(device),
            ])
