...
class SparseCascadePlanner:
    ...
    def __init__(
        ...
        # Freezer settings
        freezer_ttl: int = 60,         # Days frozen items last (60 days is realistic for frozen meals)
        ...
        #   - Result: ~$71/wk at 14.1% protein for Fam4 (vs $83/11.2% without)
        # At 18%+: add prefilter for harder steering
        #   - Excludes low-protein recipes, stronger density/macro scaling
        #   - Result: hits targets at 20%/25%/35% within 2pp
        _prot_target = self.config.protein_pct_target
        if _prot_target >= 15 and getattr(self.config, 'auto_protein_targeting', True):
            overrides = {}
            # ALWAYS enable density bonus at >= 15%
            if not self.config.enable_protein_density_bonus:
                overrides['enable_protein_density_bonus'] = True
            # Scale from 12% baseline (not 15%) so 15% gets meaningful steering
            _target_scale = max(0, (_prot_target - 12.0) / 15.0)
            # Density bonus: 3.0 at 15%, scales up to 8+ at 35%
            _auto_density = max(self.config.protein_density_value, 3.0 + _target_scale * 5.0)
            # Macro weight: 0.15 at 15%, scales up to 0.50 at 35%
            _auto_macro_w = max(self.config.macro_deviation_weight, 0.15 + _target_scale * 0.35)
            if self.config.protein_density_value < _auto_density:
                overrides['protein_density_value'] = _auto_density
            if self.config.macro_deviation_weight < _auto_macro_w:
                overrides['macro_deviation_weight'] = _auto_macro_w
            # Prefilter ONLY at 18%+ (at 15% it costs +$28/wk for +3% protein)
            if _prot_target >= 18 and not self.config.enable_protein_prefilter:
                overrides['enable_protein_prefilter'] = True
            # Tighten macro tolerance for high targets (less wiggle room)
            if _prot_target >= 25 and self.config.macro_tolerance_pct > 3.0:
                overrides['macro_tolerance_pct'] = 3.0
            # Widen protein band margin for extreme targets (>35%)
            if _prot_target >= 35 and self.config.protein_filter_margin < 8.0:
                overrides['protein_filter_margin'] = 8.0
            elif _prot_target >= 25 and self.config.protein_filter_margin < 6.0:
        ...
        # Use config's protein target (prefer config over constructor default)
        self.protein_pct_target = self.config.protein_pct_target if self.config.protein_pct_target != 15.0 else protein_pct_target
        self.carbs_pct_target = carbs_pct_target
        self.fat_pct_target = fat_pct_target
        self.macro_tolerance_pct = macro_tolerance_pct
        # Use ScoringConfig for fruit snack control, fallback to constructor param
        self.auto_inject_snacks = self.config.enable_fruit_snacks if hasattr(self.config, 'enable_fruit_snacks') else auto_inject_snacks
    ...
    def _refresh_recipe_static_tensors(self) -> None:
        ...
        cache_key = (
            ...
            int(self.package_index._gpu_option_prices.shape[1]) if self.package_index._gpu_option_prices is not None else -1,
        )
        ...
        self._recipe_pkg_sizes = self.package_index._gpu_sizes[ing_idx_long]
        self._recipe_pkg_prices = self.package_index._gpu_prices[ing_idx_long]
        self._recipe_pkg_option_sizes = self.package_index._gpu_option_sizes[ing_idx_long]
        self._recipe_pkg_option_prices = self.package_index._gpu_option_prices[ing_idx_long]
    ...
    def _choose_package_options(
        ...
        remainder_value_penalty_rate: Optional[torch.Tensor] = None,
        ...
        """
        safe_sizes = package_sizes.clamp(min=1.0)
        ...
        if remainder_value_penalty_rate is not None:
            risk_rate = remainder_value_penalty_rate.float()
            while risk_rate.dim() < to_buy.dim():
                risk_rate = risk_rate.unsqueeze(0)
            option_price_per_gram = package_prices / safe_sizes
            ranking_cost = ranking_cost + option_remainder * option_price_per_gram * risk_rate.unsqueeze(-1)
    ...
    def _package_spoilage_risk_rate(self, ing_idx: torch.Tensor) -> torch.Tensor:
        """Return per-ingredient fraction of leftover package value to charge."""
        if not getattr(self.config, "enable_package_spoilage_risk_cost", False):
            return torch.zeros_like(ing_idx, dtype=torch.float32, device=self.device)

        shelf_life = self.perishability.shelf_life_tensor[ing_idx.long()].float()
        risk_window = max(float(getattr(self.config, "package_spoilage_risk_window_days", 21.0)), 1.0)
        risk = ((risk_window - shelf_life) / risk_window).clamp(min=0.0, max=1.0)

        freezable = self.perishability.freezable_tensor[ing_idx.long()].float()
        freezable_mult = float(getattr(self.config, "freezable_package_spoilage_risk_multiplier", 0.25))
        risk = torch.where(freezable > 0, risk * freezable_mult, risk)
        return risk * float(getattr(self.config, "package_spoilage_risk_value", 0.75))
    ...
    def _sparse_score(
        ...
        package_risk_rate = self._package_spoilage_risk_rate(ing_idx)  # [N, MAX_NNZ]
        ...
        leftover_grams = (purchased_grams - to_buy_per_ing).clamp(min=0)
        price_per_gram = selected_prices / selected_sizes.clamp(min=1.0)
        spoilage_risk_cost = torch.round(
            (leftover_grams * price_per_gram * package_risk_rate.unsqueeze(0)).float().sum(dim=2) * 100.0
        ).float() / 100.0
        purchase_cost = purchase_cost + spoilage_risk_cost
        ...
        if effective_max_cal == 0:
            ...
            daily_cal = self.weekly_calories / 7.0
    ...
    def _compute_incremental_cost(
        ...
        spoilage_risk_cost = _Q(leftover_grams * price_per_gram * package_risk_rate.unsqueeze(0))
        ...
        cost = gross_cost + spoilage_risk_cost - leftover_value * leftover_credit_rate
    ...
    def _cost_filter_candidates(
        ...
        if N > PRE_FILTER_SIZE:
            ...
            package_risk_rate = self._package_spoilage_risk_rate(ing_idx)

            # Package cost for what we need to BUY (not full recipe)
            _, selected_sizes, selected_prices, purchased_grams, ingredient_costs = self._choose_package_options(
                to_buy,
                pkg_sizes,
                pkg_prices,
                getattr(self.config, "package_remainder_choice_penalty_per_kg", 0.0),
            ...
            leftover_grams = (purchased_grams - to_buy).clamp(min=0)
            price_per_gram = selected_prices / selected_sizes.clamp(min=1.0)
            spoilage_risk_cost = torch.round(
                (leftover_grams * price_per_gram * package_risk_rate).float().sum(dim=1) * 100.0
            ).float() / 100.0
            incremental_costs = incremental_costs + spoilage_risk_cost
        ...
        if K <= batch_size:
            ...
            frozen_count = (leftovers[:, :, 6] > 0).sum(dim=1).float().mean()  # Avg frozen items
            if frozen_count > self.config.frozen_count_threshold:
                # Penalty scales with frozen count using config base
                frozen_pressure_penalty = self.config.frozen_pressure_base * (frozen_count / 8.0)
                costs += frozen_pressure_penalty
    ...
    def _find_leftovers(
        ...
        # Meal compatibility (CORRECTED)
        # Breakfast: breakfast items only
        # Lunch: lunch OR dinner items (eat last night's dinner for lunch!)
        # Dinner: dinner items only
        if meal_type == 0:  # breakfast
            ...
            compatible = (meal_types == 2)  # dinner items only
        ...
        # Best leftover: prioritize fresh (lowest TTL) over frozen
        # Fresh items: priority = 100 - ttl (range ~90-100 for low TTL items)
        # Frozen items: priority = 50 - ttl (range ~20-50, below most fresh but selectable)
        # This ensures frozen items ARE selected when no fresh items are available
        # BUG FIX: Previously frozen items had priority=1, ALWAYS lost to fresh items
    ...
    def _decay_leftovers_with_freeze(
        ...
        if self.auto_freeze and about_to_expire.any():
            if self.freezer_capacity_kg > 0:
                ...
                capacity_remaining_kg = (self.freezer_capacity_kg - frozen_leftover_kg).clamp(min=0).unsqueeze(1)
    ...
    def _inject_fruit_snacks(
        ...
        Each occasion = 1 snack per person in the household.
        ...
        max_add = target_cal * self.config.calorie_ceiling_pct - current_cal  # Don't exceed ceiling
        ...
        while remaining_gap >= 50 and attempts < max_attempts:
            ...
            if remaining_gap >= cal_per_occasion * 0.8:  # Allow partial fill
                # Create occasion with scaled values
                occasion = {
                    'name': snack['name'],
                    'servings': num_people,
                    'calories': cal_per_occasion,
                    'fruit_g': snack['fruit_g'] * num_people,
                    'cost': snack['cost'] * num_people,
                }
                snacks_to_add.append(occasion)
                remaining_gap -= cal_per_occasion
    ...
    def _plan_with_carryover(
        ...
        for slot in range(NUM_SLOTS):
            ...
            if slot_servings == 0:
                ...
                if (slot + 1) % 3 == 0:
                    leftovers, waste = self._decay_leftovers_with_freeze(leftovers)
            ...
            lo_servings_to_eat = torch.minimum(
                lo_servings * lo_valid.float(),
                torch.full_like(lo_servings, slot_servings)
            ).sum(dim=1)  # [K] - capped at slot_servings
            lo_servings_to_eat = torch.minimum(lo_servings_to_eat, avail_servings)
            ...
            if getattr(self.config, 'enable_separate_shadow_prices', False):
                ...
                headroom = (ceiling_cal - current_day_cal_est).clamp(min=0)  # [K]
            ...
            del combined_scores, flat_scores, top_flat_idx
            ...
            # Priority: fresh first (100-ttl), then frozen (50-ttl)
            ...
            # Frozen items near expiration should compete with fresh items.
            # Without this, frozen items at TTL=7 have priority 43, while fresh at TTL=3 has priority 97.
            # The 50-point gap means frozen items ALWAYS lose until they actually expire.
            # Fix: Add up to +60 boost for frozen items within urgency window, making them competitive.
            #
            ...
            expiry_urgency_boost = torch.where(
                frozen_near_expiry,
                60.0 * (1.0 - lo_ttl / urgency_days).clamp(min=0),  # +60 at TTL=0, +0 at urgency_days
            ...
            if getattr(self.config, 'enable_separate_shadow_prices', False):
                ...
                daily_ceiling_for_mains = daily_target_for_mains * 1.50  # 150% max per day
            ...
            for l in range(max_slots):
                ...
                # === DAILY CALORIE CHECK FOR MAIN LEFTOVER CONSUMPTION ===
                # Don't consume leftover mains if it would push us over 150% of daily target
                current_day_cal = new_daily_nutrition[:, day, 0]  # [new_K]
                slot_cal_per_srv = sorted_cal[:, l]  # [new_K] cal per serving for this slot
                projected_cal = current_day_cal + (to_eat * slot_cal_per_srv)  # [new_K]
                over_ceiling = projected_cal > daily_ceiling_for_mains  # [new_K]
                # For frontiers over ceiling, reduce to_eat to stay under
                if over_ceiling.any():
                    headroom = (daily_ceiling_for_mains - current_day_cal).clamp(min=0)  # [new_K]
                    safe_to_eat = headroom / slot_cal_per_srv.clamp(min=1.0)  # [new_K]
                    to_eat = torch.where(over_ceiling, safe_to_eat.clamp(min=0), to_eat)  # [new_K]
                ...
                if eating.any():
                    ...
                    for ki in eating_indices.tolist():
                        ...
                        srv = int(round(float(to_eat[ki].item())))
                        if srv > 0 and rid > 0:
            ...
            else:  # dinner - only dinner mains work
                meal_type_mask = (lo_meal_types_check == 2) & valid_lo & is_main
            ...
            if needs_cooking.any():
                ...
                package_risk_rate = self._package_spoilage_risk_rate(cook_ing_idx)
                num_pkg, _selected_pkg_sizes, _selected_pkg_prices, purchased_grams, ingredient_cost = self._choose_package_options(
                    to_buy,
                    pkg_sizes,
                    pkg_prices,
                    getattr(self.config, "package_remainder_choice_penalty_per_kg", 0.0),
                ...
                # Reset TTL for purchased ingredients (newly bought items are fresh)
                # When we buy packages, reset TTL to shelf life and mark as not frozen
                # Simplified: just set to fresh TTL (fresh items typically have higher TTL anyway)
                purchased_mask = num_pkg > 0  # [num_cook, MAX_NNZ]
                if purchased_mask.any():
                    fresh_ttl = self.perishability.shelf_life_tensor[cook_ing_idx.long()]  # [num_cook, MAX_NNZ]
                    # Use flat indexing for GPU-efficient update
                    flat_ttl = new_pantry_ttl.view(-1)
                    flat_frozen = new_pantry_frozen.view(-1)
                    # Set TTL for purchased ingredients to fresh value
                    update_mask = purchased_mask.flatten()  # [num_cook * MAX_NNZ]
                    update_idx = flat_idx.flatten()[update_mask]
                    update_val = fresh_ttl.flatten()[update_mask]
                    flat_ttl.index_copy_(0, update_idx, update_val)
                    flat_frozen.index_fill_(0, update_idx, False)
            ...
            if meal_idx == 0:
                ...
                side_compatible = (side_lo_meal_types == 2)
            ...
            template_match = category_match | (side_lo_template_ids == 0) | no_template_main  # [K, L]
            ...
            side_expiry_boost = torch.where(
                ...
                60.0 * (1.0 - side_lo_ttl / urgency_days).clamp(min=0),
            ...
            if getattr(self.config, 'enable_separate_shadow_prices', False):
                ...
                daily_ceiling_for_sides = daily_target_for_sides * 1.50  # 150% max per day
            ...
            for l in range(max_side_slots):
                ...
                # === DAILY CALORIE CHECK FOR SIDE LEFTOVER CONSUMPTION ===
                # Don't consume leftover sides if it would push us over 150% of daily target
                current_day_cal = new_daily_nutrition[:, day, 0]  # [new_K]
                slot_cal_per_srv = side_sorted_cal[:, l]  # [new_K] cal per serving for this slot
                projected_cal = current_day_cal + (to_eat * slot_cal_per_srv)  # [new_K]
                over_ceiling = projected_cal > daily_ceiling_for_sides  # [new_K]
                # For frontiers over ceiling, reduce to_eat to stay under
                if over_ceiling.any():
                    headroom = (daily_ceiling_for_sides - current_day_cal).clamp(min=0)  # [new_K]
                    safe_to_eat = headroom / slot_cal_per_srv.clamp(min=1.0)  # [new_K]
                    to_eat = torch.where(over_ceiling, safe_to_eat.clamp(min=0), to_eat)  # [new_K]
            ...
            has_side_leftovers = side_total_consumed >= side_servings_needed - 0.1
            ...
            if meal_idx == 0:  # breakfast
                ...
                side_inv_mask = (side_lo_meal_types == 2) & is_side_leftover & (side_lo_servings > 0.1)
            ...
            if template_to_frontiers:
                ...
                if all_side_indices_list:
                    ...
                    daily_cal_for_cap = self.weekly_calories / 7.0
                    ...
                    for tid, frontier_list in template_to_frontiers.items():
                        ...
                        side_package_risk_rate = self._package_spoilage_risk_rate(side_ing_idx_long)
                        side_num_pkg, _side_selected_sizes, _side_selected_prices, side_purchased_grams, side_ingredient_cost = self._choose_package_options(
                            side_to_buy,
                            side_pkg_sizes,
                            side_pkg_prices,
                            getattr(self.config, "package_remainder_choice_penalty_per_kg", 0.0),
                            side_package_risk_rate,
                        )
                        ...
                        if side_purchased.any():
                            fresh_ttl = self.perishability.shelf_life_tensor[side_ing_idx_long]  # [MAX_NNZ]
                            fresh_ttl_expanded = fresh_ttl.unsqueeze(0).expand_as(side_num_pkg)
                            update_idx = side_flat_idx[side_purchased]
                            new_pantry_ttl.view(-1).index_copy_(0, update_idx, fresh_ttl_expanded[side_purchased])
                            new_pantry_frozen.view(-1).index_fill_(0, update_idx, False)
                        ...
                        if self.config.enable_second_side:
                            ...
                            if gap_ratio < self.config.side2_calorie_gap_threshold:
                                ...
                                if side2_mask.sum() > 0:
                                    ...
                                    side2_package_risk_rate = self._package_spoilage_risk_rate(side2_ing_idx_long)
                                    side2_num_pkg, _side2_selected_sizes, _side2_selected_prices, side2_purchased_grams, side2_ingredient_cost = self._choose_package_options(
                                        side2_to_buy,
                                        side2_pkg_sizes,
                                        side2_pkg_prices,
                                        getattr(self.config, "package_remainder_choice_penalty_per_kg", 0.0),
                                        side2_package_risk_rate,
                                    )
            ...
            if dedupe_enabled and new_K > self.K:
                ...
                for idx, plan in enumerate(new_selections):
                    sig_parts = []
                    for sel in plan:
                        main = int(sel[0] or 0) if len(sel) > 0 else 0
                        side1 = int(sel[1] or 0) if len(sel) > 1 else 0
                        side2 = int(sel[2] or 0) if len(sel) > 2 else 0
            ...
            del pantries, nutrition, food_groups, cost, leftovers
            ...
            if (slot + 1) % 3 == 0:
                ...
                frozen_expired = expired & pantry_frozen  # Frozen items that expired (60-day TTL ran out)
        ...
        # Extract best frontier. The beam score keeps calorie-aware candidates
        # alive during the week, but the accumulated `cost` here is the actual
        # shopping cost. Without a terminal calorie selector, the cheapest
        # surviving frontier can underfeed the household.
        final_score = cost
        if getattr(self.config, 'enable_terminal_calorie_selection', True):
            ...
            final_score = cost + under_gap_cal * under_weight + over_gap_cal * over_weight
            ...
            with open(trace_file, 'a', newline='') as _f_trace:
                ...
                for _rank, _idx_t in enumerate(trace_order.tolist(), start=1):
                    ...
                    for _sel in selections[_idx_t]:
                        _main = int(_sel[0] or 0) if len(_sel) > 0 else 0
                        _side1 = int(_sel[1] or 0) if len(_sel) > 1 else 0
                        _side2 = int(_sel[2] or 0) if len(_sel) > 2 else 0
        ...
        pantry_flow = {
            ...
            'frozen_spoilage_kg': best_frozen_spoilage, # Frozen items that expired after 60-day TTL
